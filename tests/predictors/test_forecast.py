...
def test_forecast_engine_forecast_unknown_skill():
    ...
    fr = engine.predict("unknown_skill_xyz").ok()
    assert fr is None


def test_forecast_engine_forecast_all():
    engine = SkillForecastEngine()
    freqs = {"python": 80, "java": 60, "rust": 30, "typescript": 50}
    engine.fit(freqs)
...
def test_prophet_engine_fit_returns_ok():
    snapshots = [
        Snapshot(date(2025, 1, 1), {"python": 50, "sql": 40}),
        Snapshot(date(2025, 2, 1), {"python": 60, "sql": 50}),
        Snapshot(date(2025, 3, 1), {"python": 70, "sql": 60}),
        Snapshot(date(2025, 4, 1), {"python": 80, "sql": 70}),
    ]
    engine = ProphetForecastEngine()
...
def test_prophet_engine_predict():
    snapshots = [
        Snapshot(date(2025, 1, 1), {"python": 50}),
        Snapshot(date(2025, 2, 1), {"python": 60}),
        Snapshot(date(2025, 3, 1), {"python": 70}),
        Snapshot(date(2025, 4, 1), {"python": 80}),
    ]
    engine = ProphetForecastEngine()
...
def test_prophet_engine_predict_unknown_skill():
    ...
    engine = ProphetForecastEngine()
    engine.fit(snapshots, fallback_freqs={"python": 50})
    result = engine.predict("unknown_skill")
    assert isinstance(result, Err)


def test_prophet_engine_fallback_to_genetic():
    """Skills with < 2 history points use flat (no-growth) forecast."""
    snapshots = [Snapshot(date(2025, 1, 1), {"rust": 30})]
    engine = ProphetForecastEngine()
    engine.fit(snapshots, fallback_freqs={"rust": 30})
    assert "rust" in engine._models or (engine._fallback_engine and "rust" in engine._fallback_engine._models)
...
def test_prophet_engine_top_growing():
    snapshots = [
        Snapshot(date(2025, 1, 1), {"python": 50, "sql": 40, "java": 30}),
        Snapshot(date(2025, 2, 1), {"python": 60, "sql": 50, "java": 30}),
        Snapshot(date(2025, 3, 1), {"python": 70, "sql": 60, "java": 30}),
    ]
    engine = ProphetForecastEngine()
    engine.fit(snapshots)
