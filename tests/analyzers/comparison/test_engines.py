...
class TestBM25Engine:
    def test_fit_and_compare(self):
        ...
        res = e.compare(["python", "sql"], ["python developer", "sql database"])
        assert res["score"] > 0
        assert len(res["matches"]) == 2

    def test_compare_without_fit_returns_zero(self):
        e = BM25Engine()
        res = e.compare(["python"], ["python developer"])
        assert res["score"] == 0.0
    ...
    def test_compare_no_matches(self):
        ...
        e.fit(["golang", "rust"])
        res = e.compare(["python"], ["golang"])
        # might have some BM25 score due to different tokens
        assert "score" in res

    def test_normalization(self):
        e = BM25Engine()
        e.fit(["python", "python advanced", "python expert"])
        res = e.compare(["python expert"], ["python expert"])
...
class TestEnsembleEngine:
    ...
    def test_compare_single_engine(self):
        ...
        mock_engine.compare.return_value = {
    ...
    def test_compare_multi_engine_weighted(self):
        ...
        e1.compare.return_value = {"score": 0.8, "matches": [{"skill": "py", "similarity": 0.8}]}
