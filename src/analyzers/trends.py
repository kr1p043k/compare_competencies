...
class TrendAnalyzer:
    ...
    def __init__(self, current_frequencies: dict[str, float], historical_dir: Path | None = None):
    ...
    def plot_timeline(
        ...
        for idx, (skill, points) in enumerate(timeline.items()):
            ...
            ax.plot(dates, freqs, marker="o", linewidth=2.5, markersize=10, color=colors[idx], label=skill, alpha=0.85)
    ...
    def plot_trending(self, top_n: int = 15, save_path: Path | None = None, previous_snapshot: dict = None):
        ...
        if rising:
            ...
            for bar, val in zip(bars, values_r, strict=False):
                ax1.text(
                    bar.get_width() + 0.5, bar.get_y() + bar.get_height() / 2, f"{val:.1f}%", va="center", fontsize=10
                )
        else:
        ...
        if falling:
            ...
            for bar, val in zip(bars, values_f, strict=False):
                ax2.text(
                    bar.get_width() - 0.5,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--min-change", type=float, default=3.0, help="Минимальный процент изменения для тренда (default 3.0)"
    )
    ...
    analyzer.plot_trending(
        top_n=args.top, save_path=output_dir / "trending_skills.png", previous_snapshot=previous_snapshot
    )
