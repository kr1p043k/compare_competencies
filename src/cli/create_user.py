...
Usage:
    python -m src.cli create-user email@example.com password --role admin
"""
...
async def main(email: str, password: str, role: str, full_name: str) -> None:
    async with async_session_factory() as session:
        ...
        if existing.scalar_one_or_none():
            ...
            return
        result = await session.execute(sa_text("SELECT crypt(:pw, gen_salt('bf')) AS pw_hash"), {"pw": password})
