...
def main() -> None:
    ...
    p.add_argument("--role", default="teacher", choices=["admin", "teacher", "student", "rop"])
    ...
    p.set_defaults(func=lambda a: asyncio.run(embeddings.main(force=a.force)))

    p = sub.add_parser("import-students", help="Импорт студентов из CSV")
    p.add_argument("csv", help="Path to CSV file")
    p.set_defaults(func=lambda a: asyncio.run(import_students.main(a.csv)))

    p = sub.add_parser("export-json", help="Экспорт БД в JSON")
    p.set_defaults(func=lambda a: asyncio.run(export_json.main()))
    ...
    p.add_argument("--force", action="store_true")
    p.set_defaults(func=lambda a: asyncio.run(compute_competency_vectors.main(force=a.force)))
    ...
    p.add_argument("--all", action="store_true", help="Прогнать все направления последовательно")
    p.set_defaults(func=lambda a: asyncio.run(
        map_ksa_to_skills.run_all(json_map_path=Path(a.json_map) if a.json_map else None)
        if a.all else
        map_ksa_to_skills.run_mapping(
